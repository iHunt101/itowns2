...
function FeatureToolBox(ellipsoid) {
    this.ellipsoid = ellipsoid;
}

const geoCoord = new GeoCoordinate();
...
FeatureToolBox.prototype.geoCoordTo3D = function geoCoordTo3D(longitude, latitude, altitude) {
    geoCoord.set(longitude, latitude, altitude, UNIT.DEGREE);
...
FeatureToolBox.prototype.GeoJSON2Box = function GeoJSON2Box(features, pointOrder) {
    ...
    roofMat.uniforms.lightOn.value = false;
...
FeatureToolBox.prototype.GeoJSON2Point = function GeoJSON2Point(features, bbox, layer, pointOrder) {
    ...
    points.material.uniforms.lightOn.value = false;

    for (let i = 0; i < features.length; i++) {
        const point = new NodeMesh();
        const feature = features[i];
        const coords = feature.geometry.coordinates;
        const geoCoord = new GeoCoordinate(coords[pointOrder.long], coords[pointOrder.lat], ((bbox.bottom() + bbox.top()) / 2) + 3, UNIT.DEGREE);
...
FeatureToolBox.prototype.createRasterImage = function createRasterImage(bbox, features) {
    ...
    const size = new THREE.Vector2(bbox.dimension.x, bbox.dimension.y).divideScalar(Math.PI / 180);
